...
def main():
    ...
    parser.add_argument(
        "-y",
        "--yamlpath",
        help="Yaml relative path from windspeed root",
        required=False,
        type=str,
        default="",
    )
    parser.add_argument(
        "-f",
        "--fastout",
        help="OpenFAST output file name",
        required=False,
        type=str,
        default="",
    )
    ...
    fig = plt.figure(constrained_layout=True,figsize=(12,int(nws*4)))
    subfigs = fig.subfigures(nrows=nws, ncols=1)
    ...
    for i,c in enumerate(sort_case_list):
        ...
        yaml_inp = c + '/' + args.yamlpath
        yaml = YAML(typ='safe',pure=True)

        # Read YAML file
        with open(yaml_inp, 'r') as stream:
            inp_loaded = yaml.load(stream)
        ...
        # Load openfast data
        tc = "cat " + c + "/" + args.fastout + " | awk 'length($0) > 300' > " + c + "/trimmed.out"
        print(tc)
        os.system(tc)
        ...
        if nws > 1:
            ...
            subfigs.suptitle(str(ws[i])+' m/s')
            ax = subfigs.subplots(nrows=1, ncols=len(var_list))
